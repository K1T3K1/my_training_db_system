...
void parser::handle_user_input() {
  ...
  query parsed_query{get_command_from_string(stringed_query[0]), std::span<std::string_view>()};
  ...
  parsed_queries = std::vector<query>();
  ...
  for (auto& word : stringed_query) {
    if (get_command_from_string(word) == NONE) {
      ...
      parsed_query._args = std::span<std::string_view>(_args_vector);
...
void parser::create_expressions() {
